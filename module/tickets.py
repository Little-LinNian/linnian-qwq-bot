...
async def sendmsg(event: MessageEvent, rs: Relationship):
    if event.message.as_display().startswith("#许愿券 "):
        ...
        ctx = event.message.get_first(Text).text[5:]
        if ctx.startswith("购买 "):
            ticketnumber = str(int(ctx[3:]))
            if len(tickets) == 5:
                await rs.exec(
                    MessageSend(
                        MessageChain.create([Text("许愿券超过5张了UWU，请发送#许愿券 开奖")])
                    )
                )
                pass
            if int(ticketnumber) < 0:
                await rs.exec(
                    MessageSend(MessageChain.create([Text("不能为负数哦 UwU")]))
                )
                pass
            elif not len(ticketnumber) == 6:
                await rs.exec(
                    MessageSend(MessageChain.create([Text("许愿券码为6位数 UwU")]))
                )
                pass

            elif rs.ctx.id in tickets.keys():
                await rs.exec(
                    MessageSend(MessageChain.create([Text("你已经购买了哦 UwU")]))
                )
                pass
            elif ticketnumber in tickets.values():
                await rs.exec(
                    MessageSend(MessageChain.create([Text("已经被人购买了哦 UwU")]))
                )
                pass
                ...
                try:
                    ...
                    await rs.exec(
                        MessageSend(MessageChain.create([Text("余额不足哦 UwU")]))
                    )
        if ctx.startswith("开奖"):
            if len(tickets) != 5:
                await rs.exec(
                    MessageSend(MessageChain.create([Text("许愿券还没有开奖 UwU")]))
                )
            else:
                ...
                await rs.exec(
                    MessageSend(MessageChain.create([Text("已向许愿成功的人转入60霖念币")]))
                )
        ...
        if ctx.startswith("查询 "):
            jid = int(ctx[3:])
            await rs.exec(
                MessageSend(MessageChain.create([Text(last_result[jid])]))
            )
