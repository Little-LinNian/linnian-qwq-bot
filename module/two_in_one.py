...
async def handle_message(event: MessageEvent, rs: Relationship[MemberProfile, GroupProfile]):
    ...
    data = {"api_key":config["facekey"],"api_secret":config["facesecret"],"template_url":image1,"merge_url":image2}
    async with aiohttp.ClientSession() as session:
        async with session.post("https://api-cn.faceplusplus.com/imagepp/v1/mergeface",data=data) as resp:
            resp_data = await resp.json()
            if "result" in resp_data:
                result = resp_data["result"]
                result_bytes = base64.b64decode(result)
                await rs.exec(MessageSend(MessageChain.create([PlainText("转基因成功 uwu"),Image(RawProvider( result_bytes))])))
            if "error_message" in resp_data:
                await rs.exec(MessageSend(MessageChain.create([PlainText("转基因失败 qwq\n"),PlainText(resp_data["error_message"])])))
