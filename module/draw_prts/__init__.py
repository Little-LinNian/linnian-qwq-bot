...
async def sendmsg(event: MessageEvent, rs: Relationship[MemberProfile, GroupProfile]):
    if event.message.as_display() == "#方舟单抽":
        ...
        try:
            await rs.exec(MessageSend(MessageChain.create([PlainText("qwq 将消耗6霖念币")])))
            await asyncio.sleep(2)
            await bank.withdraw(rs.ctx.id, 6)
        except Exception as e:
            await rs.exec(
                MessageSend(
                    MessageChain.create([PlainText("qwq 消耗6霖念币失败，错误信息：{}".format(e))])
                )
            )
            return
        ...
        try:
            await rs.exec(
                MessageSend(
                    MessageChain.create([PlainText("qwq 将消耗60霖念币")]),
                )
            )
            await asyncio.sleep(2)
            ...
            await rs.exec(
                MessageSend(
                    MessageChain.create([PlainText("qwq 消耗60霖念币失败，错误信息：{}".format(e))])
                )
            )
            return
