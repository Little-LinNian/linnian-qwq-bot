...
async def handle_message(
    ...
    if event.message.as_display().startswith("#i2lm add"):
        ...
        await rs.exec(
            MessageSend(MessageChain.create([PlainText(f"{name}已经被添加到使用名单中 uwu")]))
        )
        ...
        if rs.ctx.id not in ABLE_USE_LIST:
            await rs.exec(
                MessageSend(MessageChain.create([PlainText("你还没有被添加到使用名单中 uwu")]))
            )
            return
        else:
            ...
            if not event.message.has(Image):
                await rs.exec(
                    MessageSend(MessageChain.create([PlainText("你好像什么图片都没发送 awa")]))
                )
            else:
                ...
                async with aiohttp.request("GET", image) as resp:
                    ...
                    await rs.exec(
                        MessageSend(
                            MessageChain.create([PlainText("你的图片已经收到啦，请等待图片转换为链接 awa")])
                        )
                    )
                    ...
                    await rs.exec(
                        MessageSend(
                            MessageChain.create(
                                [PlainText(f"你的图片已经转换为链接，请点击{resp}查看图片 awa")]
                            )
                        )
                    )
