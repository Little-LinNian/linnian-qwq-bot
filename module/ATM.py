...
async def sendmsg(event: MessageEvent, rs: Relationship[MemberProfile, GroupProfile]):
    if event.message.as_display().startswith("#储蓄罐 "):
        if not await limiter.group_limiter_head(event,rs,__file__,5):
            return
        ...
        if msg == "注册":
            ...
            try:
                await bank.transfer(rs.ctx.id,toid,int(msg[3:]))
        ...
        await limiter.group_limiter_tail(event,__file__)
