            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                async with session.get(url, timeout=7) as response:
                    async with aiofiles.open(
                        DRAW_PATH + f"/draw_card/{path}/{codename}.png", "wb"
                    ) as f:
...
async def generate_img(
        ...
        if game_name == "azur":
            if os.path.exists(DRAW_PATH + f"/draw_card/{game_name}/{x.star}_star.png"):
                background_list.append(
                    DRAW_PATH + f"/draw_card/{game_name}/{x.star}_star.png"
                )
        pyname = cn2py(x.name)
    ...
    A = CreateImg(w, h)
    A.paste(card_img)
    A.save(DRAW_PATH + f"/draw_card/{game_name}/{datetime.now}.png")
    return DRAW_PATH + f"/draw_card/{game_name}/{datetime.now}.png"
...
async def init_up_char(announcement):
    ...
    if _CURRENT_CHAR_POOL_TITLE:
        POOL_IMG += MessageChain.create(
            [Image.fromLocalFile(up_char_dict[x[0]]["pool_img"])]
        )
    try:
        if _CURRENT_ARMS_POOL_TITLE:
            POOL_IMG += MessageChain.create(
                [Image.fromLocalFile(up_char_dict[x[1]]["pool_img"])]
            )
