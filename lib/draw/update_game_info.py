            ...
            async with session.get(url, timeout=7) as response:
                ...
                for tr in trs[1:]:
                    ...
                    for key in info_list:
                        ...
                        member_dict = intermediate_check(
                            member_dict, key, game_name, td
                        )
                    avatar_img = await _modify_avatar_url(
                        session, game_name, member_dict["名称"]
                    )
...
def _find_last_tag(element: bs4.element.Tag, attr: str, game_name: str) -> str:
    ...
    if (
        game_name not in ["pretty", "pretty_card", "guardian"]
        and last_tag.find("http") == -1
    ):
...
# 获取大图（小图快爬）
async def _modify_avatar_url(
    session: aiohttp.ClientSession, game_name: str, char_name: str
):
    if game_name == "prts":
        async with session.get(
            f"https://wiki.biligame.com/arknights/{char_name}", timeout=7
        ) as res:
            soup = BeautifulSoup(await res.text(), "lxml")
            try:
                img_url = str(soup.find("img", {"class": "img-bg"})["srcset"]).split(
                    " "
                )[-2]
            except KeyError:
    ...
    if game_name == "pretty_card":
        async with session.get(
            f"https://wiki.biligame.com/umamusume/{char_name}", timeout=7
        ) as res:
            soup = BeautifulSoup(await res.text(), "lxml")
            img_url = (
                soup.find("div", {"class": "support_card-left"})
                .find("div")
                .find("img")
                .get("src")
            )
...
async def _last_check(data: dict, game_name: str, session: aiohttp.ClientSession):
    ...
    if game_name == "genshin":
        for key in data.keys():
            async with session.get(
                f"https://wiki.biligame.com/ys/{key}", timeout=7
            ) as res:
                soup = BeautifulSoup(await res.text(), "lxml")
                _trs = ""
...
# 对抓取每行数据是否需要额外处理？
def intermediate_check(
    member_dict: dict, key: str, game_name: str, td: bs4.element.Tag
):
    if game_name == "prts":
        if key == "获取途径":
...
def get_tbody(soup: bs4.BeautifulSoup, game_name: str, url: str):
    ...
    if game_name == "guardian_arms":
        if url[-2:] == "盾牌":
            div = soup.find("div", {"class": "resp-tabs-container"}).find_all(
