    ...
    try:
        ...
        for i in range(len(times)):
            times[i] = (
                ...
                + "-"
                + times[i].split("日")[0].strip().replace("月", "-")
            )
...
class PrtsAnnouncement:
                ...
                for li in ol:
                    ...
                    if itype == "活动":
                        ...
                        async with session.get(
                            f"https://ak.hypergryph.com{a}", headers=headers, timeout=7
                        ) as res:
    ...
    async def update_up_char(self):
        ...
        try:
            ...
            contents = [
                x for x in content.contents if x.text or str(x).find("img") != -1
            ]
            start_index = -1
            end_index = -1
            ...
            for p in contents[2:]:
                ...
                if r:
                    ...
                    probability = (
                        probability if int(probability) > 10 else f"权{probability}"
                    )
                    for char in chars:
                        if char.strip():
    ...
    async def update_up_char(self):
        ...
        try:
            ...
            for table in tables:
                ...
                data[itype]["title"] = str(pool_img["title"]).split(
                    f'期{"角色" if itype == "char" else "武器"}'
                )[0][:-3]
            ...
            async with session.get(pretty_url, timeout=7) as res:
                ...
                for div in divs:
                    ...
                    if title.find("新角色追加") != -1:
                        ...
                        break
            async with session.get(
                f"https://wiki.biligame.com/{url}", timeout=7
            ) as res:
                        ...
                        try:
                            ...
                            for center in context.find_all("center"):
                                try:
                                    ...
                                    if (
                                        img
                                        and str(img["alt"]).find("新马娘") != -1
                                        and str(img["alt"]).find("总览") == 1
                                    ):
                                        data["char"]["pool_img"] = img["src"]
            ...
            for star in data["card"]["up_char"].keys():
                for name in list(data["card"]["up_char"][star].keys()):
                    ...
                    for x in all_data.keys():
                        if (
                            all_data[x]["名称"].find(tp_name) != -1
                            and all_data[x]["关联角色"] == char_name
                        ):
...
class GuardianAnnouncement:
    ...
    async def update_up_char(self):
        ...
        try:
            ...
            soup = BeautifulSoup(text, "lxml")
            context = soup.select(
                "div.col-sm-3:nth-child(3) > div:nth-child(2) > div:nth-child(1) "
                "> div:nth-child(2) > div:nth-child(3) > font:nth-child(1)"
            )[0]
