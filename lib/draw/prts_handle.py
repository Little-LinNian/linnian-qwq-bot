...
async def prts_draw(count: int = 300):
    ...
    (
        ...
        star_list,
        six_index_list,
    ) = format_card_information(count, star_list)
    up_list = []
    tmp = ""
    ...
    # return pool_info + MessageChain.create([Image.fromLocalFile(
    #   await generate_img(operator_list, 'prts', star_list))]) \
...
def _get_operator_card(add: float):
    star = get_star(
        [6, 5, 4, 3], [PRTS_SIX_P + add, PRTS_FIVE_P, PRTS_FOUR_P, PRTS_THREE_P]
    )
    if _CURRENT_POOL_TITLE:
        ...
        if random.random() < zoom:
            up_operators = [
                x.operators for x in UP_OPERATOR if x.star == star and x.zoom < 1
            ][0]
            up_operator_name = random.choice(up_operators)
            acquire_operator = [x for x in ALL_OPERATOR if x.name == up_operator_name][
                0
            ]
        else:
            all_star_operators = [
                x
                for x in ALL_OPERATOR
            ...
            if (
                weight_up_operators
                and random.random() < 1.0 / float(len(all_star_operators)) * weight
            ):
                up_operator_name = random.choice(weight_up_operators[0])
                acquire_operator = [
                    x for x in ALL_OPERATOR if x.name == up_operator_name
                ][0]
            else:
                acquire_operator = random.choice(all_star_operators)
    else:
        acquire_operator = random.choice(
            [
                x
                for x in ALL_OPERATOR
                if x.star == star and not any([x.limited, x.event_only, x.recruit_only])
...
async def _init_up_char():
    ...
    if _CURRENT_POOL_TITLE:
        POOL_IMG = MessageChain.create(
            [Image(HttpGetProvider(up_char_dict["char"]["pool_img"]))]
        )
    ...
    for star in average_dict.keys():
        for str_zoom in average_dict[star].keys():
            ...
            UP_OPERATOR.append(
                UpEvent(
                    star=int(star), operators=average_dict[star][str_zoom], zoom=zoom
                )
            )
